...
text_Salsa_canciones %>%
  ...
  ggplot(aes(x = word, y = n)) +
    ...
    ylab("Frecuencia")+
    coord_flip()+
    ggtitle(label = 'Salsa_canciones: Conteo de palabras') -> p4
# desplegar grafico 
...
text_Salsa_canciones %>%
  count(word, sort = TRUE) %>%
  with(wordcloud(words = word, freq = n, max.words = 20, colors = 'yellow4'))
title(main = "Salsa_canciones")
```
...
text_Salsa_canciones %>%
  ...
  ggplot(aes(word, n, fill = sentiment)) +
    ...
    labs(y = "Frecuencia",
         x = NULL,
         title = "Salsa_canciones : Conteo por sentiment") +
    theme_minimal() -> p4
# desplegar grafico
grid.arrange(p3, p4, ncol  = 2)
```
...
head(text_Rock-canciones_skip_counts, n = 10)
```
...
#viz 4
set.seed(1234)
plot(gcc_Salsa_canciones, layout = layout_with_kk, vertex.color = adjustcolor(cols[kc_Salsa_canciones$membership], 0.1), vertex.frame.color = cols[kc_Salsa_canciones$membership], vertex.size = 1.6*strength(gcc_Salsa_canciones), vertex.label = NA, main = "Salsa_canciones")
```
...
grupos_Rock_canciones %>%
  ...
  head(n = 5)
```
### Salsa_canciones: Top 5 grupo mayor {-}
```{r, echo = F}
