...
## Ejemplo de Bigramas de todos los géneros
...
## Bigramas Definitivos de todos los géneros
```{r}
##### importar datos
text_Rock_canciones <- unlist(c(read_csv("Rock_canciones.txt", col_names = FALSE, show_col_types = FALSE)))
names(text_Rock_canciones) <- NULL
...
# 1) Bigramas Rock (Umbral = 3)
g_rock <- text_Rock_canciones_bi_counts %>%
  filter(weight > 2) %>%
...
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Rock con Umbral = 3"
)

# 2) Bigramas Baladas (Umbral = 3)
g_baladas <- text_Baladas_bi_counts %>%
  filter(weight > 2) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Baladas con Umbral = 3"
)

# 3) Bigramas Reggaeton (Umbral = 3)
g_reggaeton <- text_Reggaeton_bi_counts %>%
  filter(weight > 2) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Reggaeton con Umbral = 3"
)

# 4) Bigramas Salsa (Umbral = 3)
g_salsa <- text_Salsa_canciones_bi_counts %>%
  filter(weight > 2) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.frame.color = 1,
  vertex.size = 3,
  vertex.label.color = "navyblue",
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Salsa con Umbral = 3"
)
```
#### Redes con Umbral 6
```{r, fig.width=6, fig.height=6, fig.align='center'}
## Formulamos redes con un umbral distinto, de hecho, el doble que el anterior.
# Cargar librerías necesarias
suppressMessages(suppressWarnings(library(igraph)))
suppressMessages(suppressWarnings(library(dplyr)))

# 1) Bigramas Rock (Umbral > 6)
g_rock <- text_Rock_canciones_bi_counts %>%
  filter(weight > 6) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Rock con Umbral > 6"
)

# 2) Bigramas Baladas (Umbral > 6)
g_baladas <- text_Baladas_bi_counts %>%
  filter(weight > 6) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Baladas con Umbral > 6"
)

# 3) Bigramas Reggaeton (Umbral > 6)
g_reggaeton <- text_Reggaeton_bi_counts %>%
  filter(weight > 6) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Reggaeton con Umbral > 6"
)

# 4) Bigramas Salsa (Umbral > 6)
g_salsa <- text_Salsa_canciones_bi_counts %>%
  filter(weight > 6) %>%
  graph_from_data_frame(directed = FALSE)
set.seed(123)
plot(
  ...
  vertex.label.cex = 1,
  vertex.label.dist = 1,
  main = "Bigramas Salsa con Umbral > 6"
)
...
# viz 1
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = 1, vertex.frame.color = 1, vertex.size = 3, vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4)
# viz 2
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = adjustcolor('red4', 0.1), vertex.frame.color = 'red4', vertex.size = 2*strength(gcc), vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4, edge.width = 3*E(g)$weight/max(E(g)$weight))
title(main = "Componente conexa Rock Canciones", outer = T, line = -1)
```

```{r, fig.width=12, fig.height=6, fig.align='center'}
##### componente conexa mas grande de la red
labels <- ifelse(strength(gcc) > 3, V(gcc)$name, NA)
...
# viz 1
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = 1, vertex.frame.color = 1, vertex.size = 3, vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4, edge.width = 3 * E(gcc)$weight / max(E(gcc)$weight))
# viz 2
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = adjustcolor('green4', 0.1), vertex.frame.color = 'green4', vertex.size = 2*strength(gcc), vertex.label = labels, vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4, edge.width = 3*E(gcc)$weight/max(E(gcc)$weight))
...
# viz 2
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = adjustcolor('red4', 0.1), vertex.frame.color = 'red4', vertex.size = 2*strength(gcc), vertex.label = labels, vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4, edge.width = 3*E(gcc)$weight/max(E(gcc)$weight))
...
# viz 2
set.seed(123)
plot(gcc, layout = layout_with_kk, vertex.color = adjustcolor('purple4', 0.1), vertex.frame.color = 'purple4', vertex.size = 2*strength(gcc), vertex.label = labels, vertex.label.color = 'black', vertex.label.cex = 0.7, vertex.label.dist = 1.5, vertex.label.degree = -pi/4, edge.width = 3*E(gcc)$weight/max(E(gcc)$weight))
title(main = "Componente conexa Salsa", outer = T, line = -1)
```
...
```
## Skipgramas Definitivos con todos los géneros y su comparativa
### Importar datos de cada género
```{r}
##### importar datos Rock
text_Rock_canciones <- unlist(c(read_csv("Rock_canciones.txt", col_names = FALSE, show_col_types = FALSE)))
names(text_Rock_canciones) <- NULL
...
Análisis de géneros musicales colombianas. 
...
```
### Medidas de las Redes
```{r, echo = F}
# Tabla comparativa para los cuatro grafos 

tab <- cbind(
  # gcc_baladas
  c(
    mean_distance(gcc_Baladas),
    mean(degree(gcc_Baladas)),
    sd(degree(gcc_Baladas)),
    clique.number(gcc_Baladas),
    edge_density(gcc_Baladas),
    transitivity(gcc_Baladas),
    assortativity_degree(gcc_Baladas)
  ),
  # gcc_Reggaeton
  c(
    mean_distance(gcc_Reggaeton),
    mean(degree(gcc_Reggaeton)),
    sd(degree(gcc_Reggaeton)),
    clique.number(gcc_Reggaeton),
    edge_density(gcc_Reggaeton),
    transitivity(gcc_Reggaeton),
    assortativity_degree(gcc_Reggaeton)
  ),
  # gcc_Rock_canciones
  c(
    mean_distance(gcc_Rock_canciones),
    mean(degree(gcc_Rock_canciones)),
    sd(degree(gcc_Rock_canciones)),
    clique.number(gcc_Rock_canciones),
    edge_density(gcc_Rock_canciones),
    transitivity(gcc_Rock_canciones),
    assortativity_degree(gcc_Rock_canciones)
  ),
  # gcc_Salsa_canciones
  c(
    mean_distance(gcc_Salsa_canciones),
    mean(degree(gcc_Salsa_canciones)),
    sd(degree(gcc_Salsa_canciones)),
    clique.number(gcc_Salsa_canciones),
    edge_density(gcc_Salsa_canciones),
    transitivity(gcc_Salsa_canciones),
    assortativity_degree(gcc_Salsa_canciones)
  )
)
